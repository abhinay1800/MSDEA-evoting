...
import json
from flask import Flask, Response, appcontext_popped, make_response,render_template,request,redirect,session,url_for,flash
...
users=[{'name': 'abhinay', 'email': 'abhinay.dew2001@gmail.com'},
        # {'name': 'abhinay', 'email': 'shubham.dew1@gmail.com'}
        ]
...
def home(): 
    ...
    #         msgs= Message(recipients=[user['email']],body=msg, subject="imp mail", sender="DamuSir@gmail.com")
    #         con.send(msgs)
    
     #3rd
    # msg = Message('Important Mail', sender='abhinay.dew2001@gmail.com', recipients=['dewabhinay@gmail.com'])
    # msg.body="  hello MF  "
    # mail.send(msg)
...
def login():
    if request.method=="POST":
        ...
        if(voter!=None and voter.Voted==False and voter.Valid==True and pwd_context.verify(Password, hashed)):     #to check that the voter cast only one vote and is validated by admin
            ...
            email=voter.email
            print(email)
            msg = Message('Important Mail', sender='abhinay.dew2001@gmail.com', recipients=[email])
            ...
            # server.ehlo()                                                                                   #sending confirmation mail
            # server.starttls()
            # server.ehlo()
...
def signup():
    ...
    if request.method=="POST":
        ...
        try:
            ...
            res = make_response("<h1>fuck yourself</h1>")
            res.set_cookie('jio  bahu','kb')
            return res
            ...
            # for i in range(4):
            #     otp+=str(r.randint(1,9))
            # session['otp']=otp                                                        ####No session pop   #### check the session activation in other page too for another page
            # message="Your OTP for registration is {}".format(otp)
...
def setcookie():
    res = make_response("<h1>fuck yourself</h1>")
    res.set_cookie('mein hu na','SIST mkb')
    return res
